...
* Dynamic Model Injection:** The `index` route passes the server-side `models_list` dictionary to the `index.html` template. A Jinja loop (`{% for model in models %}`) pre-populates the "Select a Model" modal with HTML buttons, ensuring the frontend always reflects the available models defined in `app/or_models_list.py` without hardcoded client-side lists.
...
* Single Source of Truth: The `chats` array in `script.js` acts as the master state. It holds an array of chat objects (containing `model_id`, `pretty_name`, and `messages`). Visual elements (tabs, labels, message lists) are reactive; they update only when `activeChatIndex` is modified via the `switchChat()` function.
* Concurrency Constraints:** The frontend enforces a strict limit of **4 active conversations** at one time. Attempting to add a 5th model via the UI triggers an alert and prevents the creation of the chat object.
* Source Selection Logic:
    * The `currentContext` global object tracks user-selected citations.
    * The `renderSources()` function dynamically generates checkboxes in the UI, but **only for messages where `role === 1**` (Model responses).
    * Toggling a source checkbox adds or removes the message content from `currentContext`, using the message's index in the array as the key (e.g., `{ 1: "Response text..." }`).
    * Currently, `currentContext` is reset when switching tabs to ensure sources are specific to the active view.
